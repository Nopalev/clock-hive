...
The problem with loading wrap is if a hit to an API is failed and the state of `isLoading` is set to be true forever. The use of `Future.error()` method would make catching errors in another class is possible. Any method that has a Try and Catch block with the use of `Future.error()` upon catching error will enable new method called `catchError()` that allows catching errors when mentioned method is being used. Here is an example:
...
I am planning to add several features such as:
